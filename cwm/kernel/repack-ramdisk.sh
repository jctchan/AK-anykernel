...
if grep -q retention init.mako.rc; then
	...
	sed 's/optimal_freq.*/optimal_freq 702000/' -i init.mako.rc
	sed 's/sync_freq.*/sync_freq 918000/' -i init.mako.rc
else
