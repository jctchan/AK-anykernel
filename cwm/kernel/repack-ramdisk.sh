...
if grep -q retention init.mako.rc; then
	...
	sed 's/optimal_freq.*/optimal_freq 300000/' -i init.mako.rc
	sed 's/sync_freq.*/sync_freq 300000/' -i init.mako.rc
else
